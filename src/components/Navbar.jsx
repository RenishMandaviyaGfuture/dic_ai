...
const Navbar = ({clear,darkMode,toggleTheme}) => {
  return (
    <>
      <div className="navbar px-[250px] flex items-center justify-between h-[100px] border-bottom-[1px] border-[#374151]">
        ...
        <div className="icons flex items-center gap-[20px]">
          ...
          {darkMode ? (
            <IoMdMoon onClick={toggleTheme} size={"35px"} className="cursor-pointer p-[5px] rounded-[50%] transition-all hover:bg-[#1F2937]" />
          ):(
            <IoMdSunny onClick={toggleTheme} size={"35px"} className="cursor-pointer p-[5px] rounded-[50%] transition-all hover:bg-[#1F2937]" />
          )}
        </div>
      </div>
    </>
